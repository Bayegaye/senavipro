...
def _list_transactions(type_):
    ...
    if date_debut_str:
        try:
            q = q.filter(Transaction.date >= datetime.strptime(date_debut_str, "%Y-%m-%d").date())
        except ValueError:
            pass
    if date_fin_str:
        try:
            q = q.filter(Transaction.date <= datetime.strptime(date_fin_str, "%Y-%m-%d").date())
        except ValueError:
            pass
...
# ---------- Commandes (réservations en attente) ----------

def _disponibilites_stock():
    """Stock réellement disponible par produit = stock physique moins les
    quantités déjà réservées par des commandes en attente (non encore
    débitées du stock réel)."""
    reserves = dict(
        db.session.query(Order.product_id, func.coalesce(func.sum(Order.quantity), 0))
        .filter(Order.status == "en_attente")
        .group_by(Order.product_id)
        .all()
    )
    return {p.id: p.stock - reserves.get(p.id, 0) for p in Product.query.all()}
...
def commandes():
    if request.method == "POST":
        ...
        cdate_raw = request.form.get("date") or date.today().isoformat()
        note = request.form.get("note", "").strip()
        ...
        try:
            ...
            flash("Valeurs invalides pour le produit, la quantité ou le prix.", "danger")
            return redirect(url_for("commandes"))

        if qty <= 0 or price < 0:
            flash("La quantité et le prix doivent être positifs.", "danger")
            return redirect(url_for("commandes"))
        ...
        disponibilites = _disponibilites_stock()
        if disponibilites.get(pid, 0) < qty:
            flash(
                f"Stock disponible insuffisant pour {product.name} "
                f"(disponible : {disponibilites.get(pid, 0):g} {product.unit}, demandé : {qty:g}).",
                "danger",
            )
            return redirect(url_for("commandes"))

        client = Partner.query.filter_by(name=client_name, type="client").first()
        if not client:
            client = Partner(name=client_name, type="client")
        ...
        db.session.commit()
        flash(f"Commande enregistrée pour {client.name} ({product.name}).", "success")
        return redirect(url_for("commandes"))

    statut_filtre = request.args.get("statut", "en_attente")
    query = Order.query
    if statut_filtre != "toutes":
        query = query.filter_by(status=statut_filtre)
    liste = query.order_by(Order.created_at.desc()).limit(300).all()
...
def confirmer_commande(oid):
    commande = db.session.get(Order, oid)
    if not commande:
        abort(404)
    ...
    product = db.session.get(Product, commande.product_id)
    if not product:
        flash("Produit introuvable.", "danger")
        return redirect(url_for("commandes"))
    if product.stock < commande.quantity:
        flash(
            f"Stock réel insuffisant pour confirmer cette commande "
            f"(disponible : {product.stock:g} {product.unit}, requis : {commande.quantity:g}).",
            "danger",
        )
        return redirect(url_for("commandes"))
    ...
    product.stock -= commande.quantity
    db.session.add(Transaction(
        type="vente",
        product_id=product.id,
        partner_id=commande.client_id,
    ...
    commande.sale_id = vente.id
    db.session.commit()
    flash(f"Commande confirmée : facture {vente.numero} créée.", "success")
    return redirect(url_for("commandes", statut="en_attente"))
...
def supprimer_commande(oid):
    ...
    if commande.status == "confirmee":
        flash("Impossible de supprimer une commande déjà confirmée.", "danger")
        return redirect(url_for("commandes"))
    db.session.delete(commande)
    db.session.commit()
    flash("Commande supprimée.", "info")
    return redirect(url_for("commandes", statut=request.form.get("statut", "en_attente")))
...
def depenses():
    ...
    if date_debut:
        try:
            q = q.filter(Expense.date >= datetime.strptime(date_debut, "%Y-%m-%d").date())
        except ValueError:
            pass
    if date_fin:
        try:
            q = q.filter(Expense.date <= datetime.strptime(date_fin, "%Y-%m-%d").date())
        except ValueError:
            pass
...
def _parse_date_range():
    """Lit date_debut/date_fin dans l'URL et les convertit en objets date().

    PostgreSQL (contrairement à SQLite en local) refuse de comparer une
    colonne de type date à une chaîne de caractères sans conversion
    explicite ('operator does not exist: date >= character varying') : il
    faut donc toujours utiliser des objets date() dans les filtres, jamais
    les chaînes brutes venues de request.args."""
    debut_str = request.args.get("date_debut") or (date.today() - timedelta(days=30)).isoformat()
    fin_str = request.args.get("date_fin") or date.today().isoformat()
    try:
...
@login_required
def rapports():
    date_debut, date_fin = _parse_date_range()
